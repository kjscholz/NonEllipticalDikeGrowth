...
if keep_stress_model==0
    ...
    load('StressModels/CrustalStressGridded_SummerCoon2600gg_topo_a.mat')
    toc
    x0=0;
...
a_0 = 500; % initial dike radius (m)

%param.dl       = 1e-6; % distance over which to compute gradient (m)

% time steps
begin_time     = 0;       % initialize time
end_time       = 10/(Pe*((Pe/param.mu)^2)/param.eta);      % maximum simulation time in seconds
time_vector    = linspace(begin_time,end_time,4000); % time steps at which to compute (s)

tic
mainDikePropagate % run mainChamber.m
compute_time = toc
%% plots
figure
hold on
